...
public final class BufferedSocket implements Runnable {
	...
	/** Create the retained RT4 stream facade without creating a kernel socket. */
	public static BufferedSocket createLocal(SignLink signLink) {
		System.out.println("SINGLEPLAYER_LOCAL_SESSION: RT4_STREAM_ATTACHED");
		return new BufferedSocket(signLink);
	}
	...
	/**
	 * Keep a legacy bootstrap socket readable until a clean packet boundary, then
	 * consume the retained world protocol directly from memory. Fresh socketless
	 * sessions start with localPresentationReading already enabled.
	 */
	private boolean useLocalPresentationStream() throws IOException {
		...
		if (this.localPresentationReading) {
			if (legacyAvailable > 0) {
				...
				throw new IOException("Legacy bytes arrived after local presentation cutover");
			}
			return true;
		}
		if (Protocol.opcode != -1 || legacyAvailable > 0) {
	...
	public final void read(@OriginalArg(0) int off, @OriginalArg(1) int len, @OriginalArg(3) byte[] b) throws IOException {
		...
		if (this.localPresentationReading || this.localOnly) {
			int copied = LocalPresentationBridge.readServerBytes(b, off, len);
			if (copied != len) {
				throw new EOFException("Local presentation underflow: " + copied + " of " + len);
			}
	...
	/**
	 * Last boundary before retained RT4 gameplay bytes would enter a transport.
	 * The packet bridge gets one final chance to route the newest packet and the
	 * minimap compatibility trailer is removed before a fallback decision.
	 */
	private static int finalizeSinglePlayerGameplayWrite(byte[] src, int requestedLen) {
		...
		try {
			...
			System.err.println("SINGLEPLAYER_LOCAL_PACKET: flush-boundary fallback: " + failure);
		}
	...
	public final void write(@OriginalArg(2) byte[] src, @OriginalArg(3) int len) throws IOException {
		...
		if (this.localOnly) {
			// Anything still here is traffic the retained server had no gameplay
			// decoder for. Preserve its effective behavior locally: ignore it.
			System.err.println("SINGLEPLAYER_LOCAL_PACKET: dropped compatibility bytes=" + len);
			if (src == Protocol.outboundBuffer.data) {
