...
public final class Js5CachedResourceProvider extends Js5ResourceProvider {
	...
	private void debugIndexFailure(byte[] data, String source, RuntimeException failure) {
		int actual = data == null ? 0 : Buffer.crc32(data, data.length);
		String kind = data == null ? "NULL" : (actual == this.expectedChecksum ? "PARSE_OR_VERSION" : "CRC");
		singleplayer.SinglePlayerDebug.log("JS5_INDEX_FAIL",
				"archive=" + this.archive
						+ " source=" + source
						+ " kind=" + kind
						+ " expectedCrc=" + Integer.toUnsignedString(this.expectedChecksum)
						+ " actualCrc=" + Integer.toUnsignedString(actual)
						+ " bytes=" + (data == null ? -1 : data.length)
						+ " expectedVersion=" + this.expectedVersion
						+ " error=" + failure.getClass().getSimpleName());
	}

	private void debugGroupCrcFailure(int groupId, byte[] data, int actual, String source) {
		int expected = this.index == null || this.index.groupChecksums == null
				|| groupId < 0 || groupId >= this.index.groupChecksums.length
				? 0 : this.index.groupChecksums[groupId];
		singleplayer.SinglePlayerDebug.log("JS5_CRC_FAIL",
				"archive=" + this.archive
						+ " group=" + groupId
						+ " source=" + source
						+ " expected=" + Integer.toUnsignedString(expected)
						+ " actual=" + Integer.toUnsignedString(actual)
						+ " bytes=" + (data == null ? -1 : data.length)
						+ " netErrors=" + this.netQueue.errors
						+ " netResponse=" + this.netQueue.response);
	}

	private void debugGroupVersionFailure(int groupId, int actualVersion, String source) {
		int expectedVersion = this.index == null || this.index.groupVersions == null
				|| groupId < 0 || groupId >= this.index.groupVersions.length
				? -1 : this.index.groupVersions[groupId] & 0xFFFF;
		singleplayer.SinglePlayerDebug.log("JS5_VERSION_FAIL",
				"archive=" + this.archive
						+ " group=" + groupId
						+ " source=" + source
	...
	public final Js5Index fetchIndex() {
		...
		if (this.indexRequest instanceof Js5CacheRequest) {
			try {
				...
				debugIndexFailure(local52, "cache", local88);
				this.index = null;
				if (this.netQueue.isUrgentRequestQueueFull()) {
			...
			try {
				...
				debugIndexFailure(local52, "network", ignored);
				this.netQueue.rekey();
				this.index = null;
				if (this.netQueue.isUrgentRequestQueueFull()) {
					this.indexRequest = null;
	...
	private Js5Request fetchGroupInner(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1) {
		...
		if (!(local13 instanceof Js5CacheRequest)) {
			try {
				...
				if (this.index.groupChecksums[arg1] != local199) {
					debugGroupCrcFailure(arg1, local161, local199, "network");
					throw new RuntimeException();
				}
		...
		try {
			...
			local199 = (int) crc32.getValue();
			if (this.index.groupChecksums[arg1] != local199) {
				debugGroupCrcFailure(arg1, local161, local199, "cache");
				throw new RuntimeException();
			}
			...
			if (local385 != (this.index.groupVersions[arg1] & 0xFFFF)) {
				debugGroupVersionFailure(arg1, local385, "cache");
				throw new RuntimeException();
			}
			if (this.groupStatus[arg1] != 1) {
