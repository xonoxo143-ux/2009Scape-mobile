...
public class WorldList {
	...
	private static void installSinglePlayerWorld() {
		if (loaded && worlds != null && worlds.length == 1 && sorted != null && sorted.length == 1) {
			step = 0;
			return;
		}

		int id = Player.worldId > 0 ? Player.worldId : 1;
		WorldInfo localCountry = new WorldInfo();
		localCountry.flag = 0;
		localCountry.name = JagString.parse("Local");

		World localWorld = new World();
		localWorld.country = 0;
		localWorld.flags = 1;
		localWorld.players = 1;
		localWorld.id = id;
		localWorld.activity = JagString.parse("Single-player");
		localWorld.hostname = JagString.of("127.0.0.1");

		countries = new WorldInfo[]{localCountry};
		minId = id;
		maxId = id;
		size = 1;
		worlds = new World[]{localWorld};
		sorted = new World[]{localWorld};
		checksum = 0;
		worldPos = 0;
		errors = 0;
		step = 0;
		...
		client.worldListId = id;
		Player.worldId = id;
		System.out.println("SINGLEPLAYER_WORLDLIST: LOCAL_WORLD_READY id=" + id);
	}

	@OriginalMember(owner = "client!ql", name = "b", descriptor = "(I)I")
	public static int fetch() {
		if (Boolean.getBoolean("singleplayer")) {
			installSinglePlayerWorld();
			return 0;
		}
		try {
			if (step == 0) {
	...
	public static boolean hopWorld(@OriginalArg(0) int arg0) {
		...
		if (local3 == null) {
			...
			if (client.modeWhere != 0) {
				client.defaultPort = Player.worldId + 43594;
				client.port = client.defaultPort;
				client.alternatePort = Player.worldId + 43594;
			}
			return true;
		} else {
	...
	@OriginalMember(owner = "client!wb", name = "a", descriptor = "(IIILclient!ba;Z)I")
	public static int method4595(@OriginalArg(0) World arg0, @OriginalArg(1) int arg1, @OriginalArg(3) World arg2, @OriginalArg(4) boolean arg3) {
		if (arg1 == 1) {
			...
			return arg0.country - arg2.country;
		} else if (arg1 == 3) {
			if (arg0.activity.equals(aClass100_570)) {
				if (arg2.activity.equals(aClass100_570)) {
					return 0;
				} else {
					return arg3 ? -1 : 1;
				}
			} else if (arg2.activity.equals(aClass100_570)) {
				return arg3 ? 1 : -1;
			} else {
				return arg0.activity.method3139(arg2.activity);
			}
		} else if (arg1 == 4) {
