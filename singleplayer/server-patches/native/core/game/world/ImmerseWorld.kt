...
class ImmerseWorld : StartupListener {
    ...
    companion object {
        ...
        private val immersiveBotScheduler = Executors.newSingleThreadScheduledExecutor { runnable ->
            Thread(runnable, "ImmersiveBotScheduler").apply { isDaemon = true }
        }
        ...
        fun immerseAdventurer(){
            for (i in 0..(GameWorld.settings?.max_adv_bots ?: 50)){
                val randomDelayMs = Random.nextInt(20000, 400000).toLong()
                immersiveBotScheduler.schedule(
                    { spawn_adventurers() },
                    randomDelayMs,
