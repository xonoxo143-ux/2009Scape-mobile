...
class MainGameInterface : InterfaceListener {
    ...
    override fun defineInterfaceListeners() {
        ...
        on(TOPLEVEL_FS){player, _, _, buttonID, _, _ ->
            // In the stock networked client these buttons ultimately cause the
            // server to select the corresponding side tab. The local command
            // bridge still delivers the IF_ACTION, but there is no remote packet
            // round-trip left to perform that state change for us. Keep the
            // vanilla interface and explicitly drive the retained tab-selection
            // script from the authoritative local world.
            if (buttonID in 41..54) {
                player.interfaceManager.setViewedTab(buttonID - 41)
                return@on true
            }

            // 2009Scape revisions have used both ids for the resizable world-map
            // control. Supporting both is harmless and keeps the overlay pinned-
            // revision tolerant.
            if (buttonID == 16 || buttonID == 110)
